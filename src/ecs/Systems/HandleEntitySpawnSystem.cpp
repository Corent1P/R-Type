...
RType::HandleEntitySpawnSystem::HandleEntitySpawnSystem(std::function<std::shared_ptr<Entity>()> addEntity, std::function<void(std::shared_ptr<Entity>)> deleteEntity) : ASystem(S_ENTITY_MOB_SYSTEM, addEntity, deleteEntity), _prevTime(-1)
{
    createEntityMap();
}

RType::HandleEntitySpawnSystem::HandleEntitySpawnSystem(std::function<std::shared_ptr<Entity>()> addEntity, std::function<void(std::shared_ptr<Entity>)> deleteEntity, std::function<void(const std::basic_string<unsigned char> &message)> sendToAllClient) : ASystem(S_ENTITY_MOB_SYSTEM, addEntity, deleteEntity), _sendToAllClient(sendToAllClient)
{
    createEntityMap();
}
    ...
    for (const auto &entity : entities)
    {
        ...
        else if (entity->getComponent<EntityTypeComponent>() != nullptr && entity->getComponent<EntityTypeComponent>()->getEntityType() == RType::E_PLAYER)
        {
            ...
            if (entity->getComponent<PowerUpComponent>() != nullptr && entity->getComponent<PowerUpComponent>()->getPowerUps(RType::FORCE_POD) == true)
            {
                ...
                case F_LVL_3:
                    entity->getComponent<PowerUpComponent>()->setPowerUps(RType::FORCE_POD, false);
                    createEntity(RType::E_FORCEPOD_3, entity, std::make_pair(0, 0));
                    break;
                default:
                    break;
                }
